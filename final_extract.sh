...
# Финальный скрипт для извлечения информации о пользователях из логов
# Автор: WAYPN Team
# Версия: 1.0
...
# Извлечение данных с правильным парсингом
awk '
BEGIN {
    FS=" "
    OFS=","
...

{
    ...
    # Поиск username в правильном формате
    if (index($0, "username") > 0 && index($0, "AndreySaS") > 0) {
        current_username = "AndreySaS"
    ...
    # Поиск user_id (250264613)
    if (index($0, "id") > 0 && $0 ~ /[0-9]{9,}/) {
        for (i = 1; i <= NF; i++) {
            if ($i ~ /"id":/) {
