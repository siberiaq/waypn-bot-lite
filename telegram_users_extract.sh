...
# Скрипт для извлечения уникальных пользователей Telegram из логов бота
# Автор: WAYPN Team
# Версия: 1.0
...
# Переменные
OUTPUT_FILE="telegram_users_$(date +%Y%m%d).csv"
DAYS=365  # За все время

log "📥 Загрузка логов..."
journalctl -u waypn-bot.service --since "$DAYS days ago" --no-pager > temp_full_log.txt

log "📊 Обработка логов и извлечение уникальных пользователей Telegram..."
...
# Извлечение данных пользователей Telegram
awk '
BEGIN {
    FS=" "
    OFS=","
...

{
    ...
    # Поиск блока с информацией о пользователе Telegram (не боте)
