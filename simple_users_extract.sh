...
# Простой скрипт для извлечения уникальных пользователей Telegram
# Автор: WAYPN Team
# Версия: 1.0
...
# Переменные
OUTPUT_FILE="simple_users_$(date +%Y%m%d).csv"
DAYS=365
...

{
    ...
    # Поиск конкретного пользователя AndreySaS
    if (index($0, "AndreySaS") > 0) {
        ...
        # Ищем telegram_id в соседних строках
...
# Удаление дубликатов
sort -t',' -k1,1 "$OUTPUT_FILE" | uniq > temp_sorted.csv
