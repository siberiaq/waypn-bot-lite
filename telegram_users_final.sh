...
# Финальный скрипт для извлечения уникальных пользователей Telegram из логов
# Автор: WAYPN Team
# Версия: 1.0
...
# Переменные
OUTPUT_FILE="telegram_users_final_$(date +%Y%m%d).csv"
DAYS=365
...

{
    ...
    # Поиск блока "chat" с информацией о пользователе
    ...
    # Извлечение telegram_id из блока "chat"
    ...
    # Извлечение username из блока "chat"
    ...
    # Извлечение first_name из блока "chat"
    ...
    # Извлечение last_name из блока "chat"
    ...
    # Извлечение is_premium (может быть в блоке "from")
    ...
    # Извлечение language_code (может быть в блоке "from")
