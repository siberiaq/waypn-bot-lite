...
# Скрипт для извлечения уникальных пользователей Telegram из локального файла логов
# Автор: WAYPN Team
# Версия: 1.0
...
# Переменные
OUTPUT_FILE="local_users_$(date +%Y%m%d).csv"
INPUT_LOG="bot_logs.txt"

if [ ! -f "$INPUT_LOG" ]; then
    error "Файл логов $INPUT_LOG не найден!"
    exit 1
fi

log "📥 Обработка локального файла логов..."
log "📊 Извлечение уникальных пользователей..."
