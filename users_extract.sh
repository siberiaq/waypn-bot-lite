...
# Скрипт для извлечения уникальных пользователей бота
# Автор: WAYPN Team
# Версия: 1.0
...
# Переменные
OUTPUT_FILE="unique_users_$(date +%Y%m%d).csv"
DAYS=365  # За все время

log "📥 Загрузка логов..."
journalctl -u waypn-bot.service --since "$DAYS days ago" --no-pager > temp_full_log.txt
...
TEMP_USERS="temp_users.txt"
TEMP_SORTED="temp_sorted.txt"

# Очистка временных файлов
rm -f "$TEMP_USERS" "$TEMP_SORTED"
...

{
    ...
    # Поиск блока с информацией о пользователе
    if (index($0, "from") > 0 && index($0, "{") > 0) {
    ...
    if (index($0, "id") > 0 && $0 ~ /[0-9]{9,}/ && !index($0, "message_id") && !index($0, "file_id")) {
        for (i = 1; i <= NF; i++) {
            if ($i ~ /"id":/) {
                ...
                gsub(/"/, "", telegram_id)
                gsub(/,/, "", telegram_id)
                # Проверяем, что это не ID бота
                if (telegram_id != "8383082096" && telegram_id != "8383082096,") {
                    break
                }
            }
        }
    ...
    # Если нашли закрывающую скобку и у нас есть telegram_id, сохраняем данные
    if (index($0, "}") > 0 && telegram_id != "" && telegram_id != "8383082096") {
...
' temp_full_log.txt > "$TEMP_USERS"

# Сортировка по telegram_id и удаление дубликатов
sort -t',' -k1,1 "$TEMP_USERS" | uniq > "$OUTPUT_FILE"

# Очистка временных файлов
rm -f temp_full_log.txt "$TEMP_USERS"
